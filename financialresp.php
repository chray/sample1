...
?>
	...
	<tr>
		<td style="text-align:center;font-size:20px;">Financial Expenses</td>
	<tr>
	</table>
<form action="financialresp.php" method="post">
...
<table style="margin:4px 4px 4px 4px;width:800px;height:50px;	">
			...
			if (isset($_POST['ok'])){
				if(!empty($_POST['resp'])&&!empty($_POST['object'])&&!empty($_POST['amt'])&&!empty($_POST['ppa'])){
					if($match<=0){
						...
						$q = mysql_query("insert into financial_resp (ppa,responsibility,object,amount) values ('".$_POST['ppa']."','".$_POST['resp']."','".$_POST['object']."','".$_POST['amt']."')");
							?>
							<script>
				...
				?>	<script>
					...
					header("location:financialexp.php");
	...
	<tr>
		<?php
			$q = mysql_query("select * from financial_resp order by id desc");
				while($row = mysql_fetch_assoc($q)){
				...
				?>
					<td style="text-align:center;"><?php echo $qqq;?></td>
					...
					<a href="respdel2.php?id=<?php echo $row['id'];?>" class="g-button red" onclick="return confirm('Delete?');"><i class="icon-remove"></a></td>
	</tr>	
			<?php
				}
		?>
