...
?>
	...
	<tr>
		<td style="text-align:center;font-size:20px;">Maintainance and other operating expenses</td>
	<tr>
	</table>
<form action="maintainanceresp.php" method="post">
...
<table style="margin:4px 4px 4px 4px;width:800px;height:50px;	">
			...
			if (isset($_POST['ok'])){
				if(!empty($_POST['resp'])&&!empty($_POST['object'])&&!empty($_POST['amt'])&&!empty($_POST['ppa'])){
					if($match<=0){
						...
						$q = mysql_query("insert into maintainance_resp (ppa,responsibility,object,amt) values ('".$_POST['ppa']."','".$_POST['resp']."','".$_POST['object']."','".$_POST['amt']."')");
							?>
							<script>
	...
	<tr>
		<?php
			$q = mysql_query("select * from maintainance_resp order by id desc");
				while($row = mysql_fetch_assoc($q)){
					$qq = mysql_query("select * from ppa where id='".$row['ppa']."'");
						while($rows = mysql_fetch_assoc($qq)){
						$rowss = $rows['ppa'];
						}
				?>
					<td style="text-align:center;"><?php echo $rowss;?></td>
					<td style="text-align:center;"><?php echo $row['responsibility'];?></td>
					<td style="text-align:center;"><?php echo $row['object'];?></td>
					<td style="text-align:center;"><?php echo $row['amt'];?></td>
					<td style="width:150px;"><button class="g-button green"style="margin-right:3px;"><i class="icon-pencil"></button>
					<a href="respdel1.php?id=<?php echo $row['id'];?>" class="g-button red" onclick="return confirm('Delete?');"><i class="icon-remove"></i></a></td>
	</tr>	
			<?php
				}
		?>
