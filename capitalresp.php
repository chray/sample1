...
<form action="capitalresp.php" method="post">
...
<table style="margin:4px 4px 4px 4px;width:800px;height:50px;	">
			...
			if (isset($_POST['ok'])){
				if(!empty($_POST['resp'])&&!empty($_POST['object'])&&!empty($_POST['amt'])&&!empty($_POST['ppa'])){
					if($match<=0){
						...
						$q = mysql_query("insert into capital_resp (ppa,responsibility,object,amount) values ('".$_POST['ppa']."','".$_POST['resp']."','".$_POST['object']."','".$_POST['amt']."')");
							?>
							<script>
	...
	<tr>
		<?php
			$q = mysql_query("select * from capital_resp order by id desc");
				while($row = mysql_fetch_assoc($q)){
				...
				?>
					...
					<a href="respdel3.php?id=<?php echo $row['id'];?>"onclick="return confirm('Delete?');"class="g-button red"><i class="icon-remove"></a></td>
	</tr>			
			<?php
				}
		?>
